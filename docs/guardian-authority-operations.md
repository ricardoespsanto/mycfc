...
1. Before the first supported release, create the fixed root-only file and run the immutable-candidate provisioning command above against exact schema 004. Verify the `event=guardian_release_bind_role_provisioned` record, then run `install.sh` or the approved release service. Migration 005 must detect the staged role, grant only the cutoff routine and leave guardian intake and every guardian policy disabled. Post-migration hardening must complete before the release-bind call, and the deployment log must order `database_migrate` → `database_harden` → `guardian_release_bind` → `candidate_start`. Any failure stops before candidate start or traffic switch. Reprovisioning or password rotation is never automatic.
...
After any release containing a later database migration, assume guardian activation is invalid until new current evidence passes its preflight. Never work around a blocked status by granting the web role, changing the gate row, enabling a policy directly or reusing stale image/schema evidence.

...
After the reviewed commit is merged and CI is green, set `RELEASE_INFRA_APPLIED=true` only when a separately reviewed infrastructure plan has actually been applied or infrastructure is unchanged. Pull requests receive credential-free Terraform checks only. Retained AWS changes use a protected informational preview and a separately dispatched two-stage apply for the exact current signed and CI-green `main` SHA. In the apply run, the first job runs the full Terraform gate and displays the redacted plan, its fingerprint, and a keyed semantic HMAC. The dependent production job requests approval afterward, regenerates the plan, requires both commitments to match, applies that saved binary, rechecks that `main` has not advanced immediately before mutation, and proves the resulting plan is empty. The HMAC binds unredacted sensitive values without exposing a bare digest. The policy rejects delete, replacement, forget, import, action, deferred, unknown, and provisioner operations; allowed create and update semantics still require human review. Pushes never plan against production or apply Terraform. Set `RELEASE_PUBLISH_DEPLOY_APPROVED=true` only for the exact version and commit named in the release approval. The script creates and locally verifies a signed annotated semantic tag, pushes that tag, and dispatches `deploy.yml` for the exact main-branch SHA. Re-running the same command resumes from durable state and remote evidence. It detects an exact queued or running deployment instead of dispatching another one. A failed exact run stops at a separate retry gate; only `RELEASE_DEPLOY_RETRY_APPROVED=true` authorizes that retry.

The protected workflow independently verifies the tag object, signed commit, main ancestry, and the canonical `.github/workflows/ci.yml` push run. It verifies the application image provenance, creates a canonical manifest with the tested tree, ordered migrations and expected gate states, packages and attests that manifest as an immutable registry artifact, and only then publishes the deterministic `release-<semantic-version>-<tag-time>-<SHA>` pickup tag. Internal `git-<SHA>` image builds are never eligible for production. The host remains pull-based and receives no GitHub write credential. Before any candidate command runs, it verifies both registry subjects against the exact repository, deploy workflow and source SHA, extracts the signed manifest, and matches the semantic version, image labels and release tag.
...
- `mycfc/deployment-receipt/v1` binds the manifest digest and release identity to host result, slot, failure phase, traffic-switch/rollback outcomes, observed guardian-intake and retained privacy-worker compatibility fields, and start/finish times. Both privacy-worker fields are permanently `false`.

The host writes its manifest and receipt atomically below `/etc/mycfc/deployment` and sends only the allowlisted receipt summary to `/mycfc/production/deployment`. Already-current runs emit a fresh exact receipt. GitHub re-assumes the short-lived operations-observer role after publication and accepts only a receipt created after that publication attempt and within the bounded freshness window. It rejects any receipt that reports a privacy worker or activation requirement. A separate job with no AWS identity then records the immutable publication plus an append-only, timestamp-and-hash-named receipt on the GitHub release and adds a new hash-bound comment only to explicitly supplied issue IDs.
...
The protected production deployment requires the operations observer. Set `operations_observer_enabled=true` and the exact GitHub OIDC provider ARN in both protected plan and apply `TF_VARS`, then configure its output as `AWS_OPERATIONS_OBSERVER_ROLE_ARN` in the protected production environment. Terraform prevents accidental destruction after provisioning. The role has a one-hour maximum session and no IAM user or access key. Human trust is limited to exact IAM Identity Center permission-set role ARNs; MFA must be enforced in Identity Center. Workflow trust is limited to the exact GitHub repository and protected production-environment OIDC subject. Its permissions boundary and inline policy allow only deployment-log, ECR inventory and alarm reads, while explicitly denying secrets, Parameter Store, S3 access, ECR/log mutation and role chaining.

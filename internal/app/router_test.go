...
func TestLandingRedirectsAuthenticatedVisitors(t *testing.T) {
	router := newRouter(routerPinger{}, scs.New(), handlers.Landing{}, handlers.Login{}, handlers.Registration{}, handlers.Auth{}, handlers.Dashboard{}, handlers.Repair{}, handlers.Events{})
	request := httptest.NewRequest(http.MethodGet, "/", nil)
...
func newTestRouter(pinger routerPinger, landing handlers.Landing, login handlers.Login, registration handlers.Registration, auth handlers.Auth, dashboard handlers.Dashboard) http.Handler {
	...
	return sessions.LoadAndSave(auth.Load(newRouter(pinger, sessions, landing, login, registration, auth, dashboard, handlers.Repair{}, handlers.Events{})))
}

...
The initial audit found mixed creation drawers, true dialogs, dedicated pages and inline forms without a predictable selection rule. The delivered work applies the task-surface contract across the shell, collections, administrative operations, events, announcements, ordinary and structured training, people, profiles and system states.

The final current-branch gate on 2026-08-20 passed the deterministic UI review (10 scenarios), full browser/axe suite (20 passed, 31 intentionally skipped), unit suite and Go vet. The maintained manual companion records the product-authority acceptance for the new structured-planner and photo-removal checks; it remains explicitly labelled as an assumed, non-observed screen-reader result in `docs/design/accessibility-audit.md`.

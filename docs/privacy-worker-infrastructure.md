...
The runtime order is activation check, bounded stale-upload cleanup, bounded completion recovery, one fenced category claim, ordered checkpoint execution, job completion, restore-closure export, and completion-manifest creation. Completion recovery is idempotent. `PROVIDER_RECIPIENT_NOTIFY` is routed through the real provider execution worker. The reviewed production registry remains factually empty, so a plan containing provider work still fails closed unless an evidenced registration, adapter and credential-digest key are installed through a separately reviewed policy release.

...
The candidate restores, migrates, imports/decrypts/replays v2 prescriptions and verifies subject absence through its offline replay command. Non-replayable v1 records fail the evidence run. Success creates a non-identifying HMAC-authenticated attestation tied to exact image, backup, schema and ledger digests, retains a root-only local copy, and writes a unique encrypted off-host copy for 400 days. The local attestation expires after at most 90 days and cannot be reused for another image.

...
| Training and distance leaderboard | `training_test.go`, `dashboard_test.go` | `TestDistanceLeaderboardEnforcesRankingPrivacyAndOwnership` | Administrator plan/session authoring, athlete distance reporting, Today ranking and privacy opt-out in `e2e/auth.spec.mjs`; responsive leaderboard states in `e2e/ui-review.spec.mjs` | Covered for completed-session distance ownership, period bounds, ranking ties, inactive memberships, future sessions, self/dependant privacy, and the administrator-to-athlete browser journey; replacement-session persistence still lacks browser evidence |

...
| P1 | There is no general cleanup job for tokens, outbox, consent evidence or approved operational-category periods; consent has no withdrawal/cessation/expiry anchors, and `privacy expire` is manual and handles only refused/cancelled case records. | Add the missing retention anchors plus scheduled, observable, idempotent maintenance, including future completed cases. |

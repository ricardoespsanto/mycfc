...
The provider operation is accepted only by executor/schema v2. Historical v1 plans remain readable and cannot gain provider work. A v2 plan containing `PROVIDER_RECIPIENT_NOTIFY` may use the authenticated complete-empty path only when the active approval carries the current complete zero-registration inventory and capture finds no provider targets. Otherwise the closed registry and seal-only provider target protector must be installed. Every captured service/role/contract/evidence-key/digest tuple must match the closed registry. An unknown or stale tuple aborts and rolls back the entire execution-start transaction.
...
Only the separately configured worker can list provider targets through the lease/attempt/epoch-fenced routines. For an authenticated complete-empty inventory it succeeds only with an exact zero-target capture and invokes no adapter. For a non-empty capture it opens the execution-bound envelopes, reconciles the sealed registration tuple against its own closed registry, and invokes the exact adapter with a per-call timeout and at most five local attempts (three by default). Remaining retry scheduling is owned by the existing bounded job lifecycle.

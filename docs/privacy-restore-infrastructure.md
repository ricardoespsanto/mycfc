...
The source-level restore drill is separately inert through `PRIVACY_RESTORE_DRILL_ENABLED=false`. Once approved and configured, `deployment/postgres-restore-drill.sh` chooses the oldest retained daily or monthly recovery point that passes exact-version S3/KMS checks, the v2 manifest HMAC and the encrypted-dump checksum. Unsigned v1 manifests do not count as evidence. The complete exact-version ledger is downloaded with the offline reader before isolation; the candidate image and temporary PostgreSQL then run only on a Docker internal network and receive neither AWS credentials nor SMTP/provider configuration.
...
Non-replayable v1 records and legacy v2 closures fail the current evidence run. The host independently checks the candidate result and observer aggregate, then creates the outer HMAC-authenticated attestation tied to exact image, backup, schema and ledger digests, retains a root-only local copy, and writes a unique encrypted off-host copy for 400 days. The local attestation expires after at most 90 days and cannot be reused for another image.

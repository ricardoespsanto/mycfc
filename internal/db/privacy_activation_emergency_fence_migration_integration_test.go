...
func TestPrivacyActivationEmergencyFenceForwardMigrationAppliesToPreviousBoundary(t *testing.T) {
	...
	if _, err = tx.Exec(ctx, `ALTER TABLE privacy_activation_authenticated_artifacts DROP CONSTRAINT privacy_activation_authenticated_artifacts_v7_check`); err != nil {
		t.Fatal(err)
	}
...
BEGIN
 ...
 EXECUTE replace(definition,'202609110004_guardian_authority_verification','202609110002_privacy_empty_provider_registry_activation');

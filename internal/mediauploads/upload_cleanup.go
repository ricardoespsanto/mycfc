...
content_type,size_bytes,cleanup_after,created_at FROM media_upload_cleanup_claim($1,$2)`, lease.Milliseconds(), workerRef).Scan(
...
func (s PostgresUploadCleanupStore) Complete(ctx context.Context, result UploadCleanupResult) error {
	...
	return dbgen.New(s.DB).CompleteMediaUploadCleanup(ctx, dbgen.CompleteMediaUploadCleanupParams{
...
func (s PostgresUploadCleanupStore) Fail(ctx context.Context, claim UploadCleanupClaim, workerRef uuid.UUID, retryable bool, delay time.Duration) error {
	...
	return dbgen.New(s.DB).FailMediaUploadCleanup(ctx, dbgen.FailMediaUploadCleanupParams{

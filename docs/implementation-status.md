...
| 01 Data models | Implemented source | Migrations, queries, sqlc config and transaction helper are present; season memberships, teams, modalities, and director-approved annual categories are modelled with PostgreSQL integration coverage. |
...
| 03 Dashboards | Partial | Membership-programme dashboards provide competitor metrics/training/groups and leisure news/groups; every adult account can manage dependants, and platform administrators can access fleet/repair/maintenance visibility. Club operations owns the dashboard read models through the controlled CLI/import boundary in `docs/dashboard-data-provisioning.md`; populated-state acceptance requires a successful import. Public calendar fallbacks remain available where relevant. |
...
| 05 Authentication and consent | Partial | Login, generic adult registration, guardian dependant creation with transactional responsibility consent, configuration-backed legal-document links, database-backed current-user loading, membership and platform-admin guards, logout, and administrator CLI exist; account sessions contain only identity, and administrators are platform-role grants that can coexist with memberships. |

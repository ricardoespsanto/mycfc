...
Set `TF_PLAN_HMAC_KEY` in both environments to the same independently generated 32-byte lowercase hexadecimal key. It binds sensitive plan values and the backend bucket, fixed `mycfc/production/terraform.tfstate` key, and region without publishing a guessable bare digest; a missing, malformed, or mismatched input fails closed. The calculation excludes the nondeterministic plan timestamp, canonicalizes ordering for Terraform values-representation resource/module collections and `relevant_attributes`, normalizes only the expected role-name and principal-ID difference between `github-infra-plan` and `github-infra-apply` inside `data.aws_caller_identity.current`, and excludes only Cloudflare's deprecated principal-derived `permissions` field inside the exact `data.cloudflare_zones.application` lookup. It still binds the AWS partition, account ID, session name, Cloudflare zone identity, prior-state values, and every other plan field. On a mismatch, keyed component fingerprints identify only the differing plan sections and prior-state resource addresses without revealing their contents. Keep the HMAC key separate from application secrets and rotate both copies together only when no apply run is awaiting approval.
...
The retired privacy worker, activation exchange, restore ledger and legacy-purge resources are no longer declared by this stack. Their removal is intentionally visible in the next reviewed plan and requires separate infrastructure approval. Do not import or recreate them.

The media-cleanup IAM user is the sole privacy-adjacent object-maintenance identity. It is limited to version inventory and exact version deletion under `profiles/`, `repairs/` and `equipment/`, with explicit object-read, unversioned-delete, upload and role-chaining denies. Its access key belongs only in the root-protected media-cleanup environment on the host. See [`../../../docs/privacy-rights-operations.md`](../../../docs/privacy-rights-operations.md).
...
Review the plan, then apply it. Terraform writes application runtime values into SSM under `/mycfc/production` and writes only web-runtime secrets into Secrets Manager as `/mycfc/production/app-runtime-secrets-v2`; that value does not contain the PostgreSQL owner, migration, media-cleanup or retention passwords. The retired `/mycfc/production/app-secrets` value is retained as a quarantined legacy secret because its historical versions may contain former privileged credentials, and the host runtime identity has an explicit deny against it. The new secret value is sensitive and stored in the encrypted remote Terraform state, so keep backend access tightly scoped. Stage the owner and migration passwords separately in the host's root-only `/etc/mycfc/database-control.env` for PostgreSQL and one-shot release database jobs.

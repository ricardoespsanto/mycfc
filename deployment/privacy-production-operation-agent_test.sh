...
mkdir -p "$(dirname "$2")"
printf '%s\n' '{"result":"SUCCEEDED"}' >"$2"
printf '%s\n' 'event=privacy_operation_succeeded request_id=201-1 operation=status receipt_contract=mycfc/privacy-production-operation-receipt/v1'
EOF
...
mkdir -p "$calls"
touch "$credentials"
chmod 0600 "$env_file" "$credentials" 2>/dev/null || true
cat >"$env_file" <<EOF

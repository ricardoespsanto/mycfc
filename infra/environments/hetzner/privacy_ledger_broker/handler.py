...
def _decode_request(event):
    ...
    try:
        locator_bytes = base64.b64decode(locator, validate=True)
        payload = base64.b64decode(event.get("payload", ""), validate=True)
        supplied_checksum = base64.b64decode(event.get("checksum", ""), validate=True)
    except (binascii.Error, TypeError, ValueError):
        _fail("invalid encoding")
    if len(locator_bytes) != 32 or not payload or len(payload) > MAX_PAYLOAD_BYTES or len(supplied_checksum) != 32:
...
def handler(event, _context, s3_client=None):
    ...
    key = f'{prefix}{request["kind"]}/{request["locator_hex"]}.json'
    put = {
        "Bucket": bucket,
        "Key": key,
    ...
    return {
        ...
        **({"retain_until": event["retain_until"]} if request["kind"] == "closure" else {}),
    }

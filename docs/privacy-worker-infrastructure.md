...
The runtime order is activation check, bounded stale-upload cleanup, bounded completion recovery, one fenced category claim, ordered checkpoint execution, job completion, restore-closure export, and completion-manifest creation. Completion recovery is idempotent. `PROVIDER_RECIPIENT_NOTIFY` is routed through the real provider execution worker. The reviewed production registry remains factually empty. Its zero-target checkpoint can complete only while the active approval authenticates the current complete-empty inventory and the fenced capture contains no provider targets. Any captured target still requires an evidenced registration, adapter, and credential-digest key installed through a separately reviewed policy release.
...
The reviewed empty provider source is exactly `{"contract":"mycfc/privacy-provider-registry-source/v2","inventory_scope":"SUBJECT_SPECIFIC_EXTERNAL_INTEGRATIONS","inventory_state":"COMPLETE","providers":[]}`. The producer signs this closed inventory as a v2 `READY` artifact with zero registrations. Legacy v1 emptiness, missing fields, a different scope, unknown fields, or unsupported nonempty entries are rejected. Runtime capture still fails closed if an unexpected active provider connection exists.
...
After the schema migration is installed, an independently authorized root operator must run `privacy-activation.sh provision-disable` once, and again only when deliberately rotating the disable credential. This explicit one-shot setup reads the database administrator configuration and the dedicated disable file, provisions only the fixed disable login and API grant, then exits. Routine release bootstrap, migration, and hardening jobs do not receive or rotate the disable password; hardening only reapplies the fixed API boundary when the role already exists.

The host no longer exports material for manual signing or accepts manually staged approval files. The protected `activation-ceremony-open` operation asks the database broker to create canonical material with a maximum 15-minute lifetime and uploads it to the fixed private exchange key. Separate protected GitHub environments verify the release and sign through distinct non-exportable P-256 KMS keys. The host collector retrieves only the two fixed approval keys, verifies their immutable versions and checksums, and passes the complete bundle to the broker. The exact provisioning, ceremony, recovery and rollback contract is in [`privacy-activation-exchange.md`](privacy-activation-exchange.md).
...
Activation is never performed in the web application. The root-only host broker prepares and activates an exchange, while signing remains isolated in the two GitHub environments. The exact canonical envelopes bind the proposal, ordered evidence, activation digest, release and schema, signer registry digest, role, actor, KMS key, GitHub run identity, nonce and expiry. The broker re-verifies both P-256 signatures before one atomic SQL call; PostgreSQL rebinds every envelope field, persists the exact signed bytes, rejects reused nonces, and only then records the proposal/approval and clears the kill switch. Broker credentials stay on the host and signing private keys never leave KMS.

### Emergency activation disable and incident drill

From a root shell on the application host, run `privacy-activation.sh disable`. The wrapper rejects non-root callers, any disable file that is not root-owned mode `0600`, duplicate or unknown environment entries, and noncanonical inputs. The binary independently requires effective UID 0, the exact `mycfc_privacy_activation_disable` database username with a nonempty password, the exact expected database name matching both the URL and server identity, and a canonical nonzero actor UUID. The login has no access to the public application schema and calls only `privacy_disable.privacy_activation_disable(uuid,text)` in its isolated schema. The command emits the fixed privacy-safe outcome `privacy_activation_disabled kill_switch=engaged readiness=blocked` after the database atomically confirms the switch is engaged and readiness is blocked. Every invocation starts a new disable incident, advances the switch generation, and records one event. This intentionally invalidates any approval material prepared before that invocation, including when the switch was already engaged.
...
Exercise the incident drill against `mycfc_test`, never a live subject: establish a synthetic unengaged switch, invoke the command through the restricted login, verify the switch transitions to engaged, verify worker readiness is false, invoke it again, and verify the switch generation advances again. `make test-integration` runs this database-backed drill. A production drill requires a separately approved live-system window because it deliberately revokes activation.
...
For a later separately approved production rollout: review and apply the inert worker identity; create its credential outside Terraform; review and apply exact S3, broker, and monitoring capabilities; install protected files; complete a current signed restore drill and signed infrastructure/provider/schema observations; record the four evidence items; obtain the distinct proposal and approval; then set `PRIVACY_REQUESTS_ENABLED=true`, `PRIVACY_COMPLETION_ENABLED=true`, and `PRIVACY_WORKER_ENABLED=true` and rerun the installer. Perform the dual-approved database activation before running `privacy-worker.sh readiness`, because readiness deliberately rejects an inactive database gate. After activation, verify `systemctl status mycfc-privacy-worker`, the aggregate CloudWatch stream, both alarms, and a non-production synthetic execution. Never use a live subject as a smoke test. A schema migration can invalidate release-bound evidence and leave the worker intentionally stopped; deployment records `outcome=activation-required`, installs the new immutable worker image without starting it, and succeeds only after proving the database is inactive. Fresh evidence, dual approval, activation, readiness, and worker startup are then a separate live-system change. Releases that retain valid activation stop the worker before migration and restart it only after the new application passes post-switch checks; the release log records `privacy_worker_stop`, `privacy_worker_readiness`, `privacy_worker_restart`, and `privacy_worker_verify`, and rollback restores the old environment before restarting the previous worker image.

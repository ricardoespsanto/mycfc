...
Set `TF_PLAN_HMAC_KEY` in both environments to the same independently generated 32-byte lowercase hexadecimal key. It binds sensitive plan values and the backend bucket, fixed `mycfc/production/terraform.tfstate` key, and region without publishing a guessable bare digest; a missing, malformed, or mismatched input fails closed. The calculation excludes the nondeterministic plan timestamp and normalizes only the expected role-name and principal-ID difference between `github-infra-plan` and `github-infra-apply` inside `data.aws_caller_identity.current`; it still binds the AWS partition, account ID, session name, and every other plan field. On a mismatch, keyed component fingerprints identify only the differing top-level plan sections without revealing their contents. Keep the HMAC key separate from application secrets and rotate both copies together only when no apply run is awaiting approval.

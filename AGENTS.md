...
- `make dev-bootstrap` creates and migrates both `mycfc` and `mycfc_test`, then builds browser assets. Existing volumes retain their original PostgreSQL identity; use `make reset-local` when changing local database credentials.
- `make test-integration` requires Docker services; it creates/migrates `mycfc_test`, but integration test directories are placeholders at present.

...
func newRouter(pool handlers.DBPinger, sessions *scs.SessionManager, landing handlers.Landing, login handlers.Login, registration handlers.Registration, emailVerification handlers.EmailVerification, passwordRecovery handlers.PasswordRecovery, auth handlers.Auth, dashboard handlers.Dashboard, repair handlers.Repair, events handlers.Events, announcements handlers.Announcements, training handlers.Training, structuredTraining handlers.StructuredTraining, members handlers.Members, profile handlers.Profile, news handlers.News, suggestions handlers.Suggestions, photoAlbums handlers.PhotoAlbums, foundation handlers.Foundation, polarIntegration handlers.PolarIntegration, privacyHandlers ...handlers.PrivacyRequests) http.Handler {
	...
	if privacy != nil {
		...
		mux.Handle("GET /admin/privacidade/controlo", auth.RequireAuthenticated(http.HandlerFunc(privacy.ControlLookup)))
		mux.Handle("GET /admin/privacidade/controlo/{ref}", auth.RequireAuthenticated(http.HandlerFunc(privacy.CompletionControl)))
	...
	mux.Handle("GET /perfil/dependentes/{id}/fotografia/remover", auth.RequireGuardian(http.HandlerFunc(profile.RemovePhotoPage)))
	...
	mux.Handle("GET /transicao-18", auth.RequireAuthenticated(http.HandlerFunc(dashboard.GuardianAgeHandoff)))
	mux.Handle("POST /transicao-18/email", auth.RequireAuthenticated(http.HandlerFunc(dashboard.ProposeGuardianAgeHandoffEmail)))
	...
	mux.Handle("GET /admin/fleet/equipment/{id}/retire", auth.RequireAdmin(http.HandlerFunc(dashboard.RetireEquipmentPage)))
	...
	mux.Handle("GET /admin/maintenance/{id}/complete", auth.RequireAdmin(http.HandlerFunc(dashboard.CompleteMaintenancePage)))
	...
	mux.Handle("GET /admin/membros/criar", auth.RequireAdmin(http.HandlerFunc(members.CreatePage)))
	mux.Handle("POST /admin/membros/criar", auth.RequireAdmin(http.HandlerFunc(members.Create)))
	...
	mux.Handle("GET /admin/membros/{id}/perfil/fotografia/remover", auth.RequireAdmin(http.HandlerFunc(profile.RemovePhotoPage)))
	...
	mux.Handle("GET /admin/noticias/{id}/publicar", auth.RequireAdmin(http.HandlerFunc(news.PublishPage)))
	mux.Handle("POST /admin/noticias/{id}/publicar", auth.RequireAdmin(http.HandlerFunc(news.Publish)))
	mux.Handle("GET /admin/noticias/{id}/expirar", auth.RequireAdmin(http.HandlerFunc(news.ExpirePage)))
	...
	mux.Handle("POST /dashboard/guardian/representacoes/{ref}/renovar", auth.RequireGuardian(http.HandlerFunc(dashboard.SubmitGuardianAuthorityRenewal)))
	...
	mux.Handle("GET /admin/representacoes", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAuthorityQueue)))
	mux.Handle("GET /admin/representacoes/convites", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAuthorityInvitations)))
	mux.Handle("POST /admin/representacoes/convites", auth.RequireAdmin(http.HandlerFunc(dashboard.IssueGuardianAuthorityInvitation)))
	mux.Handle("POST /admin/representacoes/convites/{ref}/revogar", auth.RequireAdmin(http.HandlerFunc(dashboard.RevokeGuardianAuthorityInvitation)))
	mux.Handle("GET /admin/representacoes/{ref}", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAuthorityDetail)))
	mux.Handle("POST /admin/representacoes/{ref}", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAuthorityTransition)))
	mux.Handle("GET /admin/transicoes-18", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAgeHandoffAdminQueue)))
	mux.Handle("GET /admin/transicoes-18/{ref}", auth.RequireAdmin(http.HandlerFunc(dashboard.GuardianAgeHandoffAdminDetail)))
	mux.Handle("POST /admin/transicoes-18/{ref}/confirmar", auth.RequireAdmin(http.HandlerFunc(dashboard.ConfirmGuardianAgeHandoffIdentity)))
	mux.Handle("POST /admin/transicoes-18/{ref}/recuperar-email", auth.RequireAdmin(http.HandlerFunc(dashboard.RecoverGuardianAgeHandoffEmail)))
	...
	mux.Handle("GET /admin/eventos/criar", auth.RequireEventStaff(http.HandlerFunc(events.CreatePage)))
	...
	mux.Handle("GET /admin/eventos/{id}/resultados", auth.RequireAdmin(http.HandlerFunc(events.ResultsLink)))
	mux.Handle("POST /admin/events/{id}/results", auth.RequireAdmin(http.HandlerFunc(events.UpdateResultsLink)))
	...
	mux.Handle("GET /admin/announcements/{id}/publicar", auth.RequireEventStaff(http.HandlerFunc(announcements.PublishPage)))
	mux.Handle("POST /admin/announcements/{id}/publish", auth.RequireEventStaff(http.HandlerFunc(announcements.Publish)))
	mux.Handle("GET /admin/announcements/{id}/expirar", auth.RequireEventStaff(http.HandlerFunc(announcements.ExpirePage)))
	...
	mux.Handle("GET /admin/treinos/planos/criar", auth.RequireEventStaff(http.HandlerFunc(training.CreatePlanPage)))
	mux.Handle("POST /admin/treinos/planos", auth.RequireEventStaff(http.HandlerFunc(training.CreatePlan)))
	mux.Handle("GET /admin/treinos/sessoes/criar", auth.RequireEventStaff(http.HandlerFunc(training.CreateSessionPage)))
	...
	mux.Handle("GET /admin/treinos/sessoes/{id}/cancelar", auth.RequireEventStaff(http.HandlerFunc(training.CancelSessionPage)))
	...
	mux.Handle("POST /treinos/sessoes/feedback", auth.RequireAuthenticated(http.HandlerFunc(training.UpdateFeedback)))
	mux.Handle("POST /treinos/sessoes/distancia", auth.RequireAuthenticated(http.HandlerFunc(training.UpdateFeedback)))
	...
	mux.Handle("GET /treinos/prescricoes/sessoes/{id}", auth.RequireFeature(featureflags.StructuredTrainingPlanning, http.HandlerFunc(structuredTraining.PrescriptionForSession)))
	mux.Handle("GET /treinos/prescricoes/{id}", auth.RequireFeature(featureflags.StructuredTrainingPlanning, http.HandlerFunc(structuredTraining.PrescriptionDetail)))
	...
	mux.Handle("POST /admin/treinos/estruturados/ciclos", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CreateCycle))))
	mux.Handle("POST /admin/treinos/estruturados/ciclos/{id}", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.UpdateCycle))))
	mux.Handle("POST /admin/treinos/estruturados/ciclos/{id}/copiar", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CopyCycle))))
	...
	mux.Handle("GET /admin/treinos/estruturados/sessoes/{session_id}/segmentos/{segment_id}/agua", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.WaterBlockTask))))
	mux.Handle("POST /admin/treinos/estruturados/sessoes/{session_id}/segmentos/{segment_id}/agua", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CreateWaterBlockTask))))
	mux.Handle("GET /admin/treinos/estruturados/sessoes/{session_id}/segmentos/{segment_id}/ginasio", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.GymBlockTask))))
	mux.Handle("POST /admin/treinos/estruturados/sessoes/{session_id}/segmentos/{segment_id}/ginasio", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CreateGymBlockTask))))
	...
	mux.Handle("POST /admin/treinos/estruturados/variacoes/grupos", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CreateVariationGroup))))
	mux.Handle("POST /admin/treinos/estruturados/variacoes", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.CreateVariation))))
	mux.Handle("POST /admin/treinos/estruturados/variacoes/{id}/retirar", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.RetireVariation))))
	mux.Handle("POST /admin/treinos/estruturados/semanas/{id}/publicar", auth.RequireFeature(featureflags.StructuredTrainingPlanning, auth.RequireEventStaff(http.HandlerFunc(structuredTraining.PublishPlan))))
	...
	mux.Handle("GET /admin/albuns/{id}/arquivar", auth.RequireContentStaff(http.HandlerFunc(photoAlbums.ArchivePage)))

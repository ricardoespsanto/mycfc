    ...
    (outbox.message_type IN ('GUARDIAN_RENEWAL_30_DAY', 'GUARDIAN_RENEWAL_7_DAY') AND EXISTS (
      ...
      WHERE reminder.id = outbox.guardian_reminder_id
        AND (relationship.id IS NULL OR guardian.id IS NULL
          ...
          OR relationship.verified_until <= $1
          OR relationship.verified_until > $1 + interval '30 days'
          ...
          OR (outbox.message_type = 'GUARDIAN_RENEWAL_30_DAY'
            AND (reminder.reminder_kind <> 'GUARDIAN_RENEWAL_30_DAY'
              OR relationship.verified_until <= $1 + interval '7 days'))
...
WITH candidate AS (
  ...
  LEFT JOIN LATERAL guardian_age_handoff_outbox_delivery(outbox.id,$1)
  ...
  WHERE ((outbox.status = 'PENDING' AND outbox.next_attempt_at <= $1)
    ...
    AND account.email = COALESCE(verification.email, reset.email)) OR (
      ...
      AND guardian_relationship.verified_until > $1
      AND guardian_relationship.verified_until <= $1 + interval '30 days'
      ...
      AND ((outbox.message_type = 'GUARDIAN_RENEWAL_30_DAY'
          ...
          AND guardian_relationship.verified_until > $1 + interval '7 days')
        OR (outbox.message_type = 'GUARDIAN_RENEWAL_7_DAY'

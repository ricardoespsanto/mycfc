...
| Member management | `members_test.go` | None | None | Unit validation only; requires seeded operator data for end-to-end evidence |
| Events | `events_test.go` | `TestListEventsForTodayRespectsMembershipCoachGrantAndAdminVisibility` | None | Unit/integration covered; authoring, RSVP, waitlist and check-in lack browser evidence |
| Announcements and official documents | `announcements_test.go` | None | None | Unit validation/authorisation only; lifecycle and first-read delivery need integration/browser coverage |
| News | `news_test.go`, `dashboard_test.go` | None | None | Unit validation/transitions only; scheduled publication needs database/browser evidence |
...
| Training | `training_test.go` | None | None | Unit scope and document validation only; plan/session/completion persistence needs integration/browser evidence |
